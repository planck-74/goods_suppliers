...
int calculateTotal(List products) {
  int total = 0;
  for (var product in products) {
    if (product is Map && product.containsKey('controller')) {
      // Get the normal price as an integer.
      int normalPrice = product['product']['price'] ?? 0;
      ...
      // Check if the product is on sale.
      bool isOnSale = product['product']['isOnSale'] ?? false;
      if (isOnSale) {
        // Retrieve the offer price if available, or fallback to the normal price.
        int offerPrice = product['product']['offerPrice'] ?? normalPrice;

        // Retrieve the maximum quantity allowed for the offer.
        int maxOfferQty =
            product['product']['maxOrderQuantityForOffer'] ?? quantity;
