...
class _OfferState extends State<Offer> {
  ...
  void initState() {
    super.initState();
    // Load Offer products using the search cubit's existing data
    // Tab index 1 represents the Offer products tab
    _loadOfferProducts();
  }

  /// Load Offer products from the search cubit
  /// This method leverages the already-fetched data instead of making new requests
  void _loadOfferProducts() {
    ...
    if (searchCubit.isDataLoaded) {
      // Data is Offer, get Offer products (tab index 1)
      searchCubit.getProductsByTabType(0);
    } else {
  ...
  void _applyClassificationFilter(String filterType, String value) {
    ...
    context.read<SearchMainStoreCubit>().filterProductsByClassification(
      ...
      filterValue: value,
      tabType: 0, // Offer products tab
    );
  }

  /// Clear all filters and show all Offer products
  /// This resets the view to show all Offer products without any filters
  void _clearAllFilters() {
    print('Clearing all filters - showing all Offer products');
    
    // Clear filter state
    _currentFilterType = null;
    _currentFilterValue = null;
    
    // Get all Offer products without any filters
    context.read<SearchMainStoreCubit>().getProductsByTabType(0);
  }
  ...
  Future<void> _refreshData() async {
    print('Refreshing Offer products data');
    
    // Refresh all data in search cubit
    await context.read<SearchMainStoreCubit>().refreshProductsData(storeId);
    
    // Reapply current filter if one exists, otherwise show all Offer products
    ...
    return Scaffold(
      body: Column(
        children: [
          ...
          // Main content area - using SearchMainStoreCubit instead of OfferCubit
          Expanded(
              ...
              child: BlocBuilder<SearchMainStoreCubit, SearchMainStoreState>(
                builder: (context, state) {
                  ...
                  // Handle loaded state
                    ...
                    final OfferProducts = state.products;
  ...
  void _showClassificationSheet() {
    showModalBottomSheet(
      ...
      builder: (BuildContext context) {
        return SheetClassificationOffer(
onClassificationSelected: _onClassificationSelected,
        ...
        // Pass the tab type (1 for Offer products)
        tabType: 0,
